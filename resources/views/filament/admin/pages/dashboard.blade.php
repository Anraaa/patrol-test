        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div class="stat-card-colorful rounded-2xl p-5 shadow-xl" style="background: linear-gradient(135deg, #3b82f6 0%, #06b6d4 100%);">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                        ...
                        <div class="flex items-baseline gap-2">
                            ...
                            @endphp
                            <span class="text-4xl font-black text-white">{{ $activeUsersCount }}</span>
                            <span class="text-xs font-semibold" style="color:rgba(186,230,253,0.85)">dari {{ count($data['users']) }}</span>
                        </div>
                    </div>
                ...
                <div class="h-1.5 w-full rounded-full overflow-hidden progress-track">
                    <div class="h-full bg-white/70 rounded-full progress-bar" style="width: {{ $activeRate2 }}%;"></div>
                </div>
            </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        <div class="w-full h-2 rounded-full overflow-hidden shadow-inner progress-track">
                            <div class="h-full rounded-full bg-white/70 progress-bar" style="width: {{ $avgCompletion }}%;"></div>
                        </div>
                    </div>
                        ...
                        @foreach($picSummary as $userId => $pic)
                            ...
                            <tr class="table-row-hover transition-colors duration-200">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl {{ $avatarSolid[$colorKey] }} text-white text-sm font-black shadow-lg">
                                            {{ $initials }}
                                        </div>
                                        <div>
                                            <div class="font-bold text-gray-900 dark:text-gray-100">{{ $pic['name'] }}</div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    ...
                                    <div class="text-xs font-semibold text-gray-400 dark:text-gray-500">dari {{ $pic['total_locations'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-xl font-black text-emerald-600 dark:text-emerald-400">{{ $pic['patrols_completed'] }}</div>
                                    <div class="text-xs font-semibold text-gray-400 dark:text-gray-500">dari {{ $pic['patrols_total'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="w-36">
                                        ...
                                        <div class="w-full h-2.5 rounded-full bg-gray-100 dark:bg-gray-700/60 overflow-hidden shadow-inner">
                                            <div class="h-full rounded-full bg-gradient-to-r {{ $cs['gradient'] }} progress-bar" style="width: {{ $progressPercent }}%;"></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
            ...
            <div class="p-5 page-bg">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($locationPerformance as $locIndex => $location)
                        ...
                        @endphp
                        <div class="loc-card group relative flex flex-col rounded-xl p-4 shadow-md loc-bg-{{ $lci }}">
                            ...
                            <div class="mb-3 pr-8">
                                <h4 class="font-black text-gray-900 dark:text-gray-100 text-sm">{{ $location['name'] }}</h4>
                            ...
                            <div class="relative w-full h-3 rounded-full overflow-hidden mb-4 shadow-inner bg-white/60 dark:bg-gray-800/50 border border-white/70 dark:border-gray-700/40">
                                <div class="h-full rounded-full bg-gradient-to-r {{ $locGrad }} shadow progress-bar"
                                     style="width: {{ $location['performance_percent'] }}%;"></div>
                            </div>
                            <div class="grid grid-cols-3 gap-2">
                                ...
                                <div class="rounded-lg p-2 shadow-sm text-center loc-stat-inner">
                                    <p class="text-sm font-black {{ $remaining > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400' }}">{{ $remaining }}</p>
            ...
            <div class="flex flex-wrap gap-2.5 p-4 page-bg">
                @foreach($picColors as $picName => $colorIdx)
                    ...
                    <div class="pic-badge flex items-center gap-2.5 rounded-xl border {{ $cs['border'] }} {{ $cs['bg'] }} pl-2 pr-4 py-1.5 shadow-md">
                        <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg {{ $avatarSolid[$colorKey] }} text-white text-[11px] font-black shadow-md flex-shrink-0">
                            {{ $initials }}
                        </span>
            ...
            <div class="grid grid-cols-7 day-header-cell">
                @foreach($dayNames as $i => $dn)
                    <div class="py-3 text-center {{ $dayHeaderColors[$i] }}">
                        <span class="text-[10px] sm:text-xs font-black uppercase tracking-widest">{{ $dn }}</span>
                    </div>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-7">
                ...
                @for($day = 1; $day <= $data['days_in_month']; $day++)
                    ...
                    <div
                        ...
                        tabindex="0"
                        role="button"
                        aria-label="Tanggal {{ $day }}, {{ $dayPatrol }} patrol selesai"
                            ...
                            @if($hasData)
                                <div class="relative flex items-center">
                                    <span class="inline-flex h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full
                                        {{ $healthStatus === 'success' ? 'bg-emerald-500' : ($healthStatus === 'danger' ? 'bg-rose-500' : 'bg-amber-400') }}
                        ...
                        <div class="space-y-1 sm:space-y-1.5">
                            @foreach($showPics as $picName => $picData)
                                ...
                                @endphp
                                <div data-pic-name="{{ $picName }}"
                                     class="pic-badge-cal flex items-center justify-between gap-1 rounded-md sm:rounded-lg border {{ $cs['border'] }} {{ $cs['bg'] }} px-1.5 sm:px-2 py-1 sm:py-1.5 transition-all duration-150">
                                    <div class="flex items-center gap-1 sm:gap-1.5 min-w-0">
                                        ...
                                        <span class="text-[9px] sm:text-[10px] font-bold truncate {{ $cs['text'] }}">{{ $shortName }}</span>
                                    </div>
                                    <div class="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
                                        <span class="text-[9px] sm:text-[10px] font-black {{ $cs['text'] }} px-1 py-0.5 rounded cal-badge-inner-bg">
                            ...
                            @if($extraCount > 0)
                                <div class="extra-count-badge flex items-center gap-1 px-1.5 sm:px-2 py-1 rounded-md sm:rounded-lg bg-indigo-50 dark:bg-indigo-900/25 border border-indigo-200 dark:border-indigo-700/50">
                                    ...
                                    <span class="text-[9px] sm:text-[10px] font-bold text-indigo-600 dark:text-indigo-400">+{{ $extraCount }}</span>
                                </div>
                            @endif
                        </div>
                        @endif
